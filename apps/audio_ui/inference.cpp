...
struct Inference::Impl
{
    ...
    {
        if (!s.aligner_directory.empty())
            return Utf8Path(s.aligner_directory);
        auto bundled = Utf8Path(s.directory) / "aligner";
        return std::filesystem::is_regular_file(bundled / "metadata.json") ? bundled : std::filesystem::path{};
    }
    void LoadAligner(const std::filesystem::path& path)
    {
        if (aligner && loaded_aligner == path)
            return;
        aligner.reset();
...
Result Inference::Run(const Settings& s, const Audio& audio, din::common::ProgressCallback progress)
{
    ...
    catch (const std::exception& e)
    {
        if (s.model != 0 || requested_aligner.empty())
            throw;
        out.warning = std::string("Qwen alignment unavailable; retrying transcription without alignment. ") + e.what();
        impl_->Load(s, false);
    }
    ...
    if (s.model != 0 && !requested_aligner.empty())
    {
        ...
        AlignTranscript(out, audio, alignment_spans,
                        [&](const Audio& slice, const std::string& text)
                        {
                            ...
                            impl_->LoadAligner(requested_aligner);
                            alignment_setup +=
    ...
    out.setup_seconds += alignment_setup;
    out.seconds = std::chrono::duration<double>(std::chrono::steady_clock::now() - start).count() - alignment_setup;
    if (progress)
    ...
    for (;;)
    {
        ...
        try
        {
            if (completion.importing)
            {
                ...
                completion.result = inference_.Run(job.settings, *job.audio,
