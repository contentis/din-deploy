...
struct Qwen3Pipeline::Impl
{
    ...
    Runtime runtime;
    AudioModel asr;
    std::unique_ptr<Runtime> alignment_runtime;
    std::unique_ptr<AlignmentEngine> aligner;
        ...
        , asr(runtime, config.model_dir, "asr")
    {
        ...
        if (!config.defer_alignment)
            PrepareAlignment();
    }

    void PrepareAlignment()
    {
        if (!config.aligner_dir.empty() && !aligner)
        {
            if (config.defer_alignment ||
                (!config.aligner_provider.empty() && config.aligner_provider != config.provider))
                alignment_runtime = std::make_unique<Runtime>(
                    config.aligner_provider.empty() ? config.provider : config.aligner_provider, config.ep_cache_dir,
                    config.ep_context_dir, config.alignment_progress ? config.alignment_progress : config.progress);
            aligner =
                std::make_unique<AlignmentEngine>(alignment_runtime ? *alignment_runtime : runtime, config.aligner_dir);
        }
    }
    ...
    TranscriptionResult TranscribeChunk(std::span<const float> audio, const std::function<void()>& wait_for_alignment)
    {
        ...
        if (wait_for_alignment)
            wait_for_alignment();
        else if (config.defer_alignment)
            PrepareAlignment();
        if (aligner)
            result.timestamps = aligner->Align(features, result.text, result.language);
        return result;
    }
...
void Qwen3Pipeline::PrepareAlignment()
{
    impl_->PrepareAlignment();
}
TranscriptionResult Qwen3Pipeline::Transcribe(const din::io::Audio& audio,
                                              const std::function<void()>& wait_for_alignment)
{
    return impl_->Transcribe(audio, wait_for_alignment);
}
